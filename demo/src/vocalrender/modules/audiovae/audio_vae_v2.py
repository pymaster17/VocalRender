...
class AudioVAE(nn.Module):
    ...
    def __init__(
        ...
        # V2 uses an asymmetric VAE: encode at ``in_sample_rate`` and decode at
        # ``out_sample_rate``. ``sample_rate`` stays as the encode-rate alias
        # for internal compatibility, but call sites should use the explicit
        # names.
        self.in_sample_rate = sample_rate
        self.out_sample_rate = out_sample_rate
        self.sample_rate = self.in_sample_rate
    ...
    def preprocess(self, audio_data, sample_rate):
        ...
        assert sample_rate == self.in_sample_rate
