...
def build_prefill_and_persistent_kpm(
    text_mask: torch.Tensor,
    feat_mask: torch.Tensor,
    max_cache_len: int,
):
    """Pad-safe key-padding masks for batched prefill + AR steps.

    Left-padded positions (neither text nor audio) must be excluded from
    attention so zero-embedding pad keys do not leak into real queries,
    which otherwise contaminates reference-audio signal and degrades voice
    cloning vs single-sample inference. AR-step positions (T, T+1, ...) are
    always real, so ``persistent_kpm`` only needs updating for the prefill
    span.
    """
    prefill_kpm = ~((text_mask.bool() | feat_mask.bool()))  # [B, T], True=pad
    B, T = prefill_kpm.shape
    persistent_kpm = torch.zeros(
        B, max_cache_len, dtype=torch.bool, device=prefill_kpm.device,
    )
    persistent_kpm[:, :T] = prefill_kpm
    return prefill_kpm, persistent_kpm


def _audio_rate_owner(obj):
    return getattr(obj, "audio_vae", obj)


def get_in_sample_rate(obj) -> int:
    owner = _audio_rate_owner(obj)
    return int(getattr(obj, "in_sample_rate", getattr(owner, "in_sample_rate")))


def get_out_sample_rate(obj) -> int:
    owner = _audio_rate_owner(obj)
    return int(getattr(obj, "out_sample_rate", getattr(owner, "out_sample_rate")))
