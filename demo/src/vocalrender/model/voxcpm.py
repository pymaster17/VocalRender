...
class VoxCPMModel(nn.Module):
    def __init__(
        ...
        # ``sample_rate`` remains the user-facing output rate alias.
    ...
    def _generate(
        ...
        if len(prompt_wav_path) == 0:
            ...
            if sr != self.in_sample_rate:
                audio = torchaudio.functional.resample(audio, sr, self.in_sample_rate)
            ...
            # (B, D, T)
            audio_feat = self.audio_vae.encode(audio.to(self.device), self.in_sample_rate).cpu()
    ...
    def _inference(
        ...
        _fsq_sample = fsq_temperature > 0.0
        enc_outputs = self.fsq_layer(enc_outputs, do_sample=_fsq_sample, fsq_temperature=fsq_temperature) * feat_mask.unsqueeze(-1) + enc_outputs * text_mask.unsqueeze(-1)
        ...
        for i in tqdm(range(max_len), disable=not verbose):
            ...
            lm_hidden = self.fsq_layer(lm_hidden, do_sample=_fsq_sample, fsq_temperature=fsq_temperature)
    ...
    def setup_batch_cache(self, batch_size: int):
        """Setup KV cache for batch inference with the specified batch size."""
    ...
    def _generate_batch(
        ...
        This method prepares batch inputs with padding and calls _inference_batch.
        ...
        self.setup_batch_cache(batch_size)

        P = self.patch_size
        D = self.audio_vae.latent_dim

        # Tokenize all texts and optionally prepend prompt audio prefix
        text_tokens_list = []   # per-sample 1-D int32 tensors
        text_mask_list = []     # per-sample 1-D int32 tensors
        audio_mask_list = []    # per-sample 1-D int32 tensors
        audio_feat_list = []    # per-sample [T_i, P, D] float tensors
        ...
        text_tokens = torch.stack(padded_tokens, dim=0).to(self.device)       # [B, T]
        text_mask = torch.stack(padded_text_masks, dim=0).to(self.device)     # [B, T]
        audio_mask = torch.stack(padded_audio_masks, dim=0).to(self.device)   # [B, T]
        audio_feat = torch.stack(padded_audio_feats, dim=0).to(               # [B, T, P, D]
        ...
        # Decode each sample's audio (and optionally capture pre-decode
        # latents shaped (T, P, D) for downstream latent dumps — this
        # avoids the lossy wav → VAE encode round trip).
        decoded_audios = []
        ...
        for i in range(batch_size):
            # Get the predicted length for this sample
            pred_len = pred_lengths[i]

            # Extract this sample's latent features (up to its actual length)
            # latent_preds shape: [B, D, T*P]
            sample_latent = latent_preds[i:i+1, :, :pred_len * P]

            if return_latents:
                flat = sample_latent.squeeze(0).to(latent_return_dtype).cpu()
                lat = flat.transpose(0, 1).contiguous().view(pred_len, P, -1)
                latents_per_sample.append(lat)

            # Decode to audio
            decode_audio = self.audio_vae.decode(sample_latent.to(torch.float32))
            decode_audio = decode_audio.squeeze(1).cpu()
            decoded_audios.append(decode_audio)

        # Reset cache to batch_size=1 for other inference calls
        self.setup_batch_cache(1)
    ...
    def _inference_batch(
        ...
        """
        Core batch inference method for audio generation.
        
        This method processes multiple samples in parallel, with each sample
        having its own stop condition.
        ...
        # Keep stop bookkeeping on-device so batched AR generation doesn't
        # force a host sync every step under the multi-GPU thread pool.
        prefix_feat_cond = feat[:, -1, ...]  # [B, P, D]
        ...
        for i in pbar:
            ...
            # Cast to buf dtype — FSDP audio eval summons params in fp32,
            # so ``feat_decoder`` may return fp32 into a bf16 buffer.
            ...
            stop_logits = self.stop_head(self.stop_actn(self.stop_proj(lm_hidden)))  # [B, 2]
            ...
            # Note: We continue updating all samples even if some stopped,
            # as we need consistent KV cache states
        ...
        # Find max prediction length for padding
        ...
        pred_feat_all = pred_feat_buf[:, :max_pred_len]  # [B, max_T, P, D]
        feat_pred = rearrange(pred_feat_all, "b t p d -> b d (t p)", b=B, p=self.patch_size)

        return feat_pred, stop_lengths.cpu().tolist()

    @classmethod
    def from_local(cls, path: str, training: bool = False, lora_config: LoRAConfig = None):
        ...
        # Auto-resize embedding if checkpoint has a larger vocab (e.g., SVS-extended checkpoint)
        ...
        if embed_key in model_state_dict:
            ...
            if ckpt_vocab_size != model_vocab_size:
                print(f"Auto-resizing embedding: {model_vocab_size} -> {ckpt_vocab_size} to match checkpoint", file=sys.stderr)
                old_emb = model.base_lm.embed_tokens
                _, emb_dim = old_emb.weight.shape
                new_emb = torch.nn.Embedding(
                    ckpt_vocab_size, emb_dim,
