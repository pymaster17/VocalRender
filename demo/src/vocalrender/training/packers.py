...
class AudioFeatureProcessingPacker:
    ...
    def __init__(
        ...
        if self.tokenizer is not None:
            self.process_functions["svs"] = self.process_svs_data
            self.task_id_map["svs"] = 2
            self._build_svs_maps()
            
        self.id_to_task = {idx: usage for usage, idx in self.task_id_map.items()}

    def _build_svs_maps(self):
        from ..model.svs_utils import get_svs_token_maps
        pitch_tokens, note_tokens, bpm_tokens, _, _ = get_svs_token_maps()
        
        # Build lookups: Value -> ID
        # Pitch: 0-127 -> ID
        self.pitch_to_id = {}
        ...
        for nt in note_tokens:
            ...
            if nid != self.tokenizer.unk_token_id:
                # <NOTE_4> -> "4"
                val_str = nt.split('_')[1][:-1]
                self.note_to_id[val_str] = nid
    ...
    def process_svs_data(self, audio_token: torch.Tensor, text_token: torch.Tensor, is_prompt: bool = False):
        """
        Process SVS (Singing Voice Synthesis) data with the aggregated prompt
        layout ``[Tok1, Tok2, ..., Pitch, Note]`` per syllable (melisma rows
        reuse the previous syllable's text tokens).

        Args:
            audio_token: Raw audio waveform tensor
            text_token: Tensor of shape [L, N] where:
                - Columns 0 to N-4: Text token IDs (variable, 0 means padding/empty)
                - Column N-3: Pitch MIDI value (0-127)
                - Column N-2: Note type value (e.g., 4 for quarter note)
                - Column N-1: BPM value (global, taken from first row)
        """
        L = text_token.shape[0]
        num_cols = text_token.shape[1]
        
        # Parse column layout: [Text..., Pitch, Note, BPM]
        # Minimum: [Text, Pitch, Note, BPM] = 4 cols
        # Variable: [Text1, Text2, ..., Pitch, Note, BPM]
        # Last 3 columns are always: Pitch, Note, BPM
        num_text_cols = num_cols - 3  # Everything except Pitch, Note, BPM
        if num_text_cols < 1:
            num_text_cols = 1  # Fallback: at least 1 text column
        ...
        # 0. Handle BPM (Global Condition from first row)
        bpm_val = 120  # Default
        if bpm_col < num_cols:
            bpm_val = int(text_token[0, bpm_col].item())
            
        if bpm_val in self.bpm_to_id:
            full_seq_ids.append(self.bpm_to_id[bpm_val])
        elif 120 in self.bpm_to_id:
            full_seq_ids.append(self.bpm_to_id[120])
        
        # Track previous text tokens for melisma detection
        prev_text_ids = None
        
        for i in range(L):
            # Extract text token IDs (filter out 0s which are padding)
            text_ids = []
            for tc in range(num_text_cols):
                tid = int(text_token[i, tc].item())
                if tid != 0:  # 0 is padding
                    text_ids.append(tid)
            
            pitch_val = int(text_token[i, pitch_col].item())
            note_val = int(text_token[i, note_col].item())
            
            # Melisma detection: same text as previous row
            is_melisma = False
            if prev_text_ids is not None:
                if text_ids == prev_text_ids:
                    is_melisma = True
            
            # Get pitch and note token IDs
            pitch_id = self.pitch_to_id.get(pitch_val, None)
            note_str = str(note_val)
            note_id = self.note_to_id.get(note_str, None)
            
            # Aggregated layout: [Tok1, Tok2, ..., Pitch, Note] per syllable;
            # melisma rows omit text tokens and just append (Pitch, Note).
            if not is_melisma:
            ...
            prev_text_ids = text_ids
        
        # Convert to tensor
        svs_seq_token = torch.tensor(full_seq_ids, dtype=torch.long, device=text_token.device)
        
        # Append Audio Start
        svs_seq_token = torch.cat(
            [
                svs_seq_token,
                torch.tensor(
        ...
        text_length = svs_seq_token.shape[0]
        ...
        # Masks
        text_mask = torch.cat([torch.ones(text_length), torch.zeros(audio_length), torch.ones(1)]).type(torch.int32).to(text_token.device)
        audio_mask = torch.cat([torch.zeros(text_length), torch.ones(audio_length), torch.zeros(1)]).type(torch.int32).to(text_token.device)
        
        # Loss mask: Only calc loss on Audio part (unless prompt)
        ...
        return (
            ...
            labels,
            audio_duration,
            L,  # approximate token count (chars/syllables)
        )
    ...
    def encode_audio(self, wav: torch.Tensor):
        ...
        with torch.no_grad():
            z = self.audio_vae.encode(wav, self.audio_vae.in_sample_rate)  # [1, D, T']
