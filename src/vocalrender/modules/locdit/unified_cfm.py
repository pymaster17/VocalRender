...
class UnifiedCFM(torch.nn.Module):
    ...
    def forward(
        ...
        if temperature_mode == "ditar":
            # DiTAR Algorithm 1: τ = temperature ∈ [0, 1]
            # τ=1 → standard ODE from N(0,I); τ=0 → greedy from 0
            if temperature >= 1.0:
                z = torch.randn((b, self.in_channels, t), device=mu.device, dtype=mu.dtype)
            else:
                z = torch.zeros((b, self.in_channels, t), device=mu.device, dtype=mu.dtype)
        else:
            # Default: scale mode — original behavior
    ...
    def solve_euler(
        ...
        # DiTAR: find the ODE step closest to τ for noise injection
        ditar_inject_step = None
        if temperature_mode == "ditar" and 0 < temperature < 1:
            # Find the step index whose t_span value is closest to τ
            ditar_inject_step = (t_span - temperature).abs().argmin().item()
        ...
        for step in range(1, len(t_span)):
            ...
            # DiTAR Algorithm 1: inject noise at the step closest to τ
            # x̂₀ = x_t - t * v_pred  (linear flow: x_t = (1-t)*x₀ + t*z)
            # x = (1-t)*x̂₀ + t*ε     (re-noise with fresh Gaussian)
            if ditar_inject_step is not None and step == ditar_inject_step:
                t_val = t.item() if t.dim() == 0 else t
                x_hat_0 = x - t_val * dphi_dt  # estimate clean data
                eps = torch.randn_like(x)
                x = (1 - t_val) * x_hat_0 + t_val * eps
