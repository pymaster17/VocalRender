...
class MiniCPMLongRoPE(nn.Module):
    ...
    def forward(self, position_ids: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            cos, sin — shape ``(seq_len, head_dim)`` for 1-D input, or
            ``(batch_size, 1, seq_len, head_dim)`` for 2-D input (head-dim
            broadcast-slot inserted so ``apply_rotary_pos_emb`` aligns with
            ``q/k`` of shape ``(B, H, S, D)``).
        """
        cos = self.cos_cached[position_ids]
        sin = self.sin_cached[position_ids]

        if cos.ndim == 3:
            # (B, S, D) → (B, 1, S, D) so cos/sin broadcast over num_heads.
            cos = cos.unsqueeze(1)
            sin = sin.unsqueeze(1)
...
class MiniCPMAttention(nn.Module):
    ...
    def forward(
        ...
        if attn_mask is not None:
            # Caller has pre-folded the causal structure into ``attn_mask``
            # (additive float mask, -inf at pad / future positions), so we
            # dispatch SDPA with ``is_causal=False``.
    ...
    def forward_step(
        ...
        # Slice KV cache to valid positions only (no attn_mask needed).
        # This allows SDPA to dispatch to FlashAttention / MemEfficient backend
        # instead of falling back to the slow Math backend.
        seq_len = (position_id.item() if isinstance(position_id, torch.Tensor) else position_id) + 1
        key_used = key_cache[:, :, :seq_len, :]
        value_used = value_cache[:, :, :seq_len, :]
        ...
        if key_padding_mask is not None:
            # Mask out pad keys from the prefill prefix. Only the prefill's
            # left-padded region can be pad here — newly-generated positions
            # are always real, so the slice ``[:, :seq_len]`` is enough.
            kpm = key_padding_mask[:, :seq_len]  # [B, seq_len], True = pad
...
class MiniCPMDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            attn_mask: optional additive [B, 1, T, T] mask (pre-folded causal
                + key-padding). When given, ``is_causal`` is ignored.
        """
...
class MiniCPMModel(nn.Module):
    ...
    def __init__(self, config: MiniCPM4Config):
        ...
        self.no_rope = getattr(config, 'no_rope', False)
        self.rope_emb = None if self.no_rope else MiniCPMLongRoPE(config)
    ...
    def forward(
        ...
        Args:
            ...
            is_causal: bool, whether the attention mask is causal
            key_padding_mask: optional Tensor(batch_size, seq_length), bool,
                True at pad positions. When provided, a combined causal +
                key-padding additive mask is built and fed to SDPA; ``is_causal``
                is ignored. Needed by batched inference with variable-length
                prefixes (e.g. ``_inference_batch``) so pad positions do not
                leak into the real tokens' attention output.
            position_ids: optional Tensor(seq_length,) or Tensor(batch_size,
                seq_length). Overrides the default ``arange`` positions.
        Returns:
        ...
        if key_padding_mask is not None:
            ...
            # Build additive [B, 1, T, T] mask = causal(T, T) with -inf above
            # diagonal PLUS -inf on pad keys across all queries.
            dtype = inputs_embeds.dtype
            device = inputs_embeds.device
            causal = torch.zeros(q_len, q_len, dtype=dtype, device=device)
            if is_causal:
                causal.masked_fill_(
                    torch.triu(
            ...
            attn_mask = causal.unsqueeze(0).unsqueeze(0).expand(bsz, 1, q_len, q_len).clone()
            if key_padding_mask is not None:
                assert key_padding_mask.shape == (bsz, q_len), (
        ...
        next_decoder_cache = [] if not use_checkpoint else None

        for decoder_layer in self.layers:
            if use_checkpoint:
                def layer_forward(hidden_states, decoder_layer=decoder_layer, attn_mask=attn_mask):
        ...
        return hidden_states, next_decoder_cache or []
    ...
    def forward_step(
        ...
        Args:
            ...
            key_padding_mask: optional Tensor(batch_size, max_length), bool,
                True at pad positions written into the KV cache during prefill.
                Propagated through every decoder layer so pad keys are
                excluded from the AR step's attention.
        Returns:
